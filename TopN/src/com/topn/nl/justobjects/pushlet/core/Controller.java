...
package com.topn.nl.justobjects.pushlet.core;
...
import com.topn.nl.justobjects.pushlet.util.PushletException;
...
public class Controller implements Protocol, ConfigDefs {
	 ...
	 * @param aCommand
	 */
	public void doServerCommand(Command aCommand) {
		try {
			// Update lease time to live
			session.kick();
			...
			if (subject == null) {
				...
				if (to != null) {
					if(!StringUtil.isBlank(to)){
						//这里应该还要判断是不是应该改单播，如果不是要将信息保存到数据库中去，将来可能要改造这个方法
						Session session = SessionManager.getInstance().getSession(to);
						if(null == session){
							
							
							//保存到数据库中去,实现我们的留言业务
							String msg = aCommand.reqEvent.getField("msg");
							if(StringUtil.isBlank(msg)){
								return;
							}
							//msg = URLDecoder.decode(msg, "utf-8");   
							msg = StringUtil.parseTOUTF_8(msg);
								
							FriendController.getInstance().leaveMsg(Integer.parseInt(aCommand.session.getId()), Integer.parseInt(to), msg);
							
							
						}else{
							Dispatcher.getInstance().unicast(aCommand.reqEvent, session);
						}						
					}
				} else {
					// No to: multicast
