...
(function($) {
	...
	$.zxxbox = function(elements, options) {
		...
		var s = $.extend({}, zxxboxDefault, options || {});
		//弹框的显示
		var WRAP = '<div id="zxxBlank" onselectstart="return false;"></div><div class="wrap_out" id="wrapOut"><div class="wrap_in" id="wrapIn"><div id="wrapBar" class="wrap_bar"  onselectstart="return false;"><div class="wrap_title"><span>'+myTitle+'</span></div><a href="javasctipt:" class="wrap_close" id="wrapClose">'+s.shut+'</a></div><div class="wrap_body" id="wrapBody"></div></div></div>';
		if ($("#wrapOut").size()) {
			$("#wrapOut").show();
			if (s.bg) {
				$("#zxxBlank").show();	
...
//fn为按下确定后的回调函数
function showAskBox(title, message, sureFn, cancelFn, cancelTitle) {
    myTitle = checkTitle(title);
    if (cancelTitle == null || cancelTitle == "") {
        cancelTitle = "取消";
    }
    if (cancelFn == undefined || cancelFn == null) {
        cancelFn = closeBox;
    }
    $.zxxbox.ask(message, function () {
        sureFn();
        closeBox();
    }, cancelFn, {
        title: "友情提示"
...
function showBox(id, title, isDrag) {
    var tt = $("body " +id).length;
	if(tt > 1){
		var temp = $("body").find(id).first();
		$("body " +id).remove();
		$("body").append(temp);
	}
    myTitle = checkTitle(title);
    if (isDrag == undefined || isDrag == null || isDrag == false) {
        $.zxxbox($(id), { drag: false });
    }
    else {
        $.zxxbox($(id), { drag: true });
    }
	...
	{
		$("#zxxBlank").append("<iframe style='position:absolute;width:100%;height:100%;_filter:alpha(opacity=0);opacity:0;border-style:none;z-index:200;'></iframe> ");
	}
}

function showMessage(title, message) {
    myTitle = checkTitle(title);
    $.zxxbox.remind(message);
}
